...
def on_encoder_touched(_, encoder_name):
    try:
        # Mackie Control gets first shot (so we send MCU touch immediately)
        if hasattr(app, "mc_mode") and app.is_mode_active(app.mc_mode):
            if hasattr(app.mc_mode, "on_encoder_touched"):
                app.mc_mode.on_encoder_touched(encoder_name)
                return
        # Otherwise let the active modes handle it if they care
        for mode in app.active_modes[::-1]:
            if hasattr(mode, "on_encoder_touched"):
...
def on_encoder_released(_, encoder_name):
    try:
        # Mackie Control gets first shot (so we send MCU touch-up immediately)
        if hasattr(app, "mc_mode") and app.is_mode_active(app.mc_mode):
            if hasattr(app.mc_mode, "on_encoder_released"):
                app.mc_mode.on_encoder_released(encoder_name)
                return
        # Otherwise let the active modes handle it if they care
        for mode in app.active_modes[::-1]:
            if hasattr(mode, "on_encoder_released"):
